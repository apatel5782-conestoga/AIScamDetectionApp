...
function buildFallbackReply(messages: ChatMessage[], userLocation?: string, uploadedFileNames: string[] = []): string {
  const latestUserMessage = getLatestUserMessage(messages);
  const fileNote = uploadedFileNames.length > 0
    ? ` I also received these files: ${uploadedFileNames.join(", ")}.`
    : "";
  ...
  if (!latestUserMessage) {
    return `I'm having trouble reaching the live AI service right now, but I can still help with general scam-safety guidance.${fileNote}${locationNote}`;
  }
  ...
  return [
    ...
    `Paste the exact message again in a moment and I'll re-check it once the AI connection is back.${fileNote}${locationNote}`,
  ].join("\n\n");
}
...
export async function chatWithAI(
  ...
  const fileContext = uploadedFileContext?.trim()
    ? `\n\nUploaded evidence files: ${uploadedFileNames.join(", ") || "attached evidence"}\n\nExtracted content from uploads:\n${uploadedFileContext}\n\nWhen files are attached, assess whether the evidence looks legitimate, suspicious, or likely a scam. Never claim certainty unless the evidence is conclusive. Use verdict labels like "Likely scam", "Suspicious", "Likely legitimate", or "Need more evidence", then explain the signals and what the user should do next.`
    : "";
  ...
  try {
    ...
    return buildFallbackReply(messages, userLocation, uploadedFileNames);
  }
}
